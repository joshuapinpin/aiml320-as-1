...
class ClassificationPipeline:
    ...
    def load_dataset(self, csv_path: Path, target_col="diagnosis") -> tuple[pd.DataFrame, pd.Series]:
    ...
    def split_data(self, X: pd.DataFrame, y: pd.Series) -> SplitData:
    ...
    def impute_data(self, split: SplitData, strategy: str) -> SplitData:
    ...
    def evaluate_imputation_strategies(self, split: SplitData, max_depth: int = 5) -> dict[str, float]:
        results = {}
    ...
    def scale_data(self, split: SplitData, scaler) -> SplitData:
        """
    ...
    def evaluate_normalisation_impact(
        ...
        conditions = {
            "No scaling": split,
            "Standardised": self.scale_data(split, StandardScaler()),
            "Min-Max scaled": self.scale_data(split, MinMaxScaler())
        }
    ...
    def compute_pearson_correlations(self, split: SplitData) -> pd.Series:
        """
    ...
    def select_features_by_correlation(self, correlations: pd.Series, threshold: float = 0.6) -> list[str]:
        """
    ...
    def apply_pca(self, split: SplitData, n_components) -> tuple[SplitData, PCA]:
        """
